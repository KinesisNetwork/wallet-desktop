...
describe('Transfer epic', () => {
  ...
  describe('transferRequest$', () => {
    ...
    it('success', (done) => {
      ...
      transferRequest$(action$, store$, { createKinesisTransfer, getActiveKeys, getCurrentConnection })
      .pipe(toArray())
      .subscribe((actions) => {
        ...
        expect(createKinesisTransfer).toHaveBeenCalledWith(privateKey, connection, transferPayload)
        done()
      })
    })
    ...
    it('failure', (done) => {
      ...
      const expectedOutputActions = [
        { type: getType(transactionFailed), payload: error },
      ]
  ...
  describe('transactionSubmission$', () => {
    ...
    it('success', (done) => {
      const expectedOutputActions = [
        { type: getType(transactionSuccess) },
      ]
      
      const getCurrentConnection = jest.fn(() => connection)
      const submitSignedTransaction = jest.fn(() => Promise.resolve())
      
      transactionSubmission$(action$, store$, { getCurrentConnection, submitSignedTransaction })
        .pipe(toArray())
        .subscribe((actions) => {
          expect(actions).toEqual(expectedOutputActions)
          expect(getCurrentConnection).toHaveBeenCalledWith(store)
  ...
  describe('transactionSuccess$', () => {
    ...
    it('success', (done) => {
      const expectedOutputActions = [
        { type: getType(accountLoadRequest), payload: publicKey },
      ]

      const generalSuccessAlert = jest.fn(() => Promise.resolve())
      const getActiveKeys = jest.fn(() => ({ publicKey }))

      transactionSuccess$(action$, store$, { generalSuccessAlert, getActiveKeys })
        .pipe(toArray())
        .subscribe((actions) => {
          expect(actions).toEqual(expectedOutputActions)
  ...
  describe('transactionFailed$', () => {
    ...
    it('failure', (done) => {
      ...
      transactionFailed$(action$, state$, { generalFailureAlert, getTransactionErrorMessage })
        .pipe(toArray())
        .subscribe((actions) => {
          expect(actions).toEqual(expectedOutputActions)
