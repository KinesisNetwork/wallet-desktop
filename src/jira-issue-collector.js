...
!(function($) {
  ...
  ;($ = jQuery.noConflict(!0)),
    'undefined' == typeof window.jQuery && (window.jQuery = $),
  ...
  ;(ATL_JQ.isQuirksMode = function() {
    ...
    (ATL_JQ.IssueDialog = function(options) {
      ...
      if (
        (options.baseUrl || (options.baseUrl = 'https://bullioncapital.atlassian.net'),
        (this.options = options),
        (this.frameUrl =
          options.baseUrl +
          '/rest/collectors/1.0/template/form/' +
          this.options.collectorId +
          '?os_authType=none'),
        $('head').append("<style type='text/css'>" + css + '</style>'),
        'CUSTOM' === this.options.triggerPosition)
      ) {
          ...
          try {
            ...
            ;(i =
              ...
              (window.ATL_JQ_PAGE_PROPS.triggerFunction ||
                (window.ATL_JQ_PAGE_PROPS['3c9704cb'] &&
                  window.ATL_JQ_PAGE_PROPS['3c9704cb'].triggerFunction))
                ? window.ATL_JQ_PAGE_PROPS.triggerFunction ||
                  window.ATL_JQ_PAGE_PROPS['3c9704cb'].triggerFunction
                : oldTriggerFunction),
              $.isFunction(i) && i(showDialog)
          } catch (i) {}
        })
      } else if ($.isFunction(this.options.triggerPosition))
        try {
          this.options.triggerPosition(showDialog)
        } catch (i) {}
      else if (this.options.triggerPosition && this.options.triggerText) {
      ...
      var $iframeContainer = $("<div id='atlwdg-container'/>").addClass(
          'atlwdg-popup atlwdg-hidden',
        ),
        $blanket = $("<div id='atlwdg-blanket' class='atlwdg-blanket'/>").hide()
      $body.append($blanket).append($iframeContainer)
      var browser = (function(i) {
        i = i.toLowerCase()
        var t = /(msie) ([\w.]+)/.exec(i) || []
        return { isIE: !!t[1], version: t[2] || '0' }
      })(navigator.userAgent)
      if (browser.isIE && (ATL_JQ.isQuirksMode() || browser.version < 9)) {
        $('head').append("<style type='text/css'>" + cssIE + '</style>')
        var triggerAdjuster = function(i) {}
        if ('TOP' === this.options.triggerPosition)
          triggerAdjuster = function(i) {
            $('#atlwdg-trigger').css('top', $(window).scrollTop() + 'px')
          }
        else if ('RIGHT' === this.options.triggerPosition)
          triggerAdjuster = function(i) {
            var t = $('#atlwdg-trigger')
            t.css(
              'top',
              $(window).height() / 2 - t.outerWidth() / 2 + $(window).scrollTop() + 'px',
            ),
              ATL_JQ.isQuirksMode() ||
                '8.0' !== browser.version ||
          ...
          triggerAdjuster = function(i) {
            var t = $(window)
            $trigger.css('top', t.scrollTop() + t.height() - outerHeight + 'px')
          }
        }
    ...
    (ATL_JQ.IssueDialog.prototype = {
      ...
      show: function() {
        var i = this,
          ...
          o = $('body'),
          e = $(
            '<iframe id="atlwdg-frame" scrolling="no" frameborder="0" src="' +
              this.frameUrl +
              '"></iframe>',
          ),
          r = 'Issue collector is loading',
          A = $(
            '<div id="atlwdg-loading" class="atlwdg-loading" title="' + r + '" tabindex="0"></div>',
          )
        t.css('height', '600px'),
          ...
          A.show(),
          A.focus(),
          o.css('overflow', 'hidden').keydown(hideDialog),
          window.scroll(0, 0)
        var n = ''
        if (this.options.collectFeedback) {
          ...
          n = []
          for (var s in g)
            g.hasOwnProperty(s) && g[s] && 'string' == typeof g[s] && n.push('*' + s + '*: ' + g[s])
          n = n.join('\n')
        }
        var a = {}
        this.options.fieldValues &&
          !$.isEmptyObject(this.options.fieldValues) &&
          $.extend(a, this.options.fieldValues),
          e.on('load', function() {
            var t = { messageType: 'collector.iframe.loaded', feedbackString: n, fieldValues: a }
            e[0].contentWindow.postMessage(JSON.stringify(t), i.options.baseUrl)
          }),
          t.show(),
          this.options.customHeight && setDialogHeight(this.options.customHeight),
          $(window).bind('message', function(o) {
            if (o.originalEvent.source === e[0].contentWindow && o.originalEvent.data)
              if ('cancelFeedbackDialog' === o.originalEvent.data) i.hide()
              else if ('collectorLoaded' === o.originalEvent.data)
                A.remove(), t.removeClass('atlwdg-hidden')
              else if (!i.options.customHeight)
                try {
        ...
        ;(d.href = 'https://bullioncapital.atlassian.net'), t.append(e), $('#atlwdg-blanket').show()
      },
      hide: function() {
        $('body')
          .css('overflow', 'auto')
          .unbind('keydown', hideDialog),
          $('#atlwdg-container')
            .addClass('atlwdg-hidden')
            .hide()
            .empty(),
          $('#atlwdg-blanket').hide()
      },
    })
  var filterStrings = function(i, t) {
    for (var o in i)
      if (i.hasOwnProperty(o)) {
        var e = i[o]
        if (void 0 === t && $.isArray(e)) filterStrings(e, o)
        else if ('string' != typeof e) {
          var r = void 0 === t ? o : t + ':' + o
          console.log(
            "bootstrap.js:filterStrings ignoring key for value '" + r + "'; typeof must be string",
          ),
            delete i[o]
        }
      }
  ...
  ATL_JQ(function() {
    var i = function(i, t) {
      if (i.enabled) {
        ...
        if (
          (window.ATL_JQ_PAGE_PROPS &&
            ((r = window.ATL_JQ_PAGE_PROPS.fieldValues),
            window.ATL_JQ_PAGE_PROPS.hasOwnProperty(t) &&
              (r = window.ATL_JQ_PAGE_PROPS[t].fieldValues),
            $.isFunction(r)
          ...
          if (window.ATL_JQ_PAGE_PROPS) {
            ...
            window.ATL_JQ_PAGE_PROPS.hasOwnProperty(t) &&
              (n = window.ATL_JQ_PAGE_PROPS[t].environment),
              $.isFunction(n) ? $.extend(A, n()) : $.extend(A, n)
          }
          e = function() {
            return A
          }
        ...
        window.ATL_JQ_PAGE_PROPS &&
          window.ATL_JQ_PAGE_PROPS.height &&
          (o = window.ATL_JQ_PAGE_PROPS.height),
          new ATL_JQ.IssueDialog({
            collectorId: t,
            fieldValues: r,
            collectFeedback: e,
            triggerText: i.triggerText,
            triggerPosition: i.triggerPosition,
            triggerFunction: i.triggerFunction,
            baseUrl: i.baseUrl,
            customHeight: o,
          })
      }
    ...
    if ('undefined' != typeof ATL_JQ_CONFIGS)
      for (var t in ATL_JQ_CONFIGS) {
        var o = ATL_JQ_CONFIGS[t]
        delete ATL_JQ_CONFIGS[t], i(o, t)
      }
    else {
      var e = 'https://bullioncapital.atlassian.net'
      $.ajax({
        url: e + '/rest/collectors/1.0/configuration/trigger/3c9704cb?os_authType=none',
        dataType: 'jsonp',
        crossDomain: !0,
        jsonpCallback: 'trigger_3c9704cb',
        cache: !0,
        success: function(t) {
          i(t, '3c9704cb')
        },
